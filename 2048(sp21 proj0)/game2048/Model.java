    ...
    public boolean tilt(Side side) {
        ...
        for(int col = 0; col < board.size(); col++){
            ...
            for(int row = board.size() - 2; row >= 0; row--){
                Tile m = board.tile(col,row);
                if(m != null){
                    int up = row + 1;
                    while(up < board.size() - 1 && board.tile(col,up) == null){
                        up++;
                    }
                    if(board.tile(col,up) == null){
                        board.move(col,up,m);
                        changed = true;
                        continue;
                    } else if(merged[up]){
                        board.move(col,up-1,m);
                        changed = true;
                        continue;
                    } else if(board.tile(col,up).value() == m.value()){
                        board.move(col,up,m);
                        merged[up] = true;
                        changed = true;
                        score += m.value() * 2;
